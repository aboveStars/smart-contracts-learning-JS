...
export class NftMarketPlace extends ethereum.SmartContract {
  ...
  getProceeeds(seller: Address): BigInt {
    let result = super.call("getProceeeds", "getProceeeds(address):(uint256)", [
  ...
  try_getProceeeds(seller: Address): ethereum.CallResult<BigInt> {
    let result = super.tryCall(
      "getProceeeds",
      "getProceeeds(address):(uint256)",
      [ethereum.Value.fromAddress(seller)]
    );
    if (result.reverted) {
      return new ethereum.CallResult();
